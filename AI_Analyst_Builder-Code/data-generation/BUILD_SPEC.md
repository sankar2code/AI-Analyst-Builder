# SampleMart Dataset Build Spec

## What This Document Is

A complete, detailed specification for building the SampleMart synthetic datasets. Everything needed to implement — database schema, generation logic, embedded data stories, data quality landmines, scripts, agents, verification queries, and build order — is defined here. No implementation decisions should need to be made outside this document.
...
**SampleMart** — a mid-stage e-commerce marketplace (Amazon-like) with:
- A delivery membership program ("SampleMart Plus")
...
| acquisition_channel | TEXT | How user found SampleMart | organic (30%), paid_search (25%), social (15%), referral (10%), email (10%), tiktok_ads (10% — but 0% before Jun 1) |
...
Phase 6 — Export practice dataset:
  ...
  └── load_duckdb.py       → creates data/practice/practice_ecommerce_practice.duckdb
...
Phase 7 — Generate capstone dataset:
  ...
  └── load_duckdb.py       → creates data/capstone/practice_ecommerce_capstone.duckdb
...
| **Plus** | SampleMart Plus membership (paid delivery/perks program) |
